...
class API:
    ...
    def request(self, method, url, body=None, *, allow404=False):
...
class Control:
    def __init__(self, config_path, root, gcloud):
    ...
    def operation(self, name, *, timeout=600):
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            result = self.api.request("GET", self.base + name)
            if result.get("done"):
                if result.get("error"):
                    raise RuntimeError(
    ...
    def guard_heartbeat(self):
        ...
        def update(last_probe_at, healthy):
            with lock:
    ...
    def _guard_loop(self, publish_heartbeat):
...
def check_guard(root, config_digest):
    root = Path(root)
    try:
...
def retain_guard_refusal(control, error):
    ...
    state = control.state()
    receipt = state.get("admission_stop")
...
def main():
    ...
    control = Control(args.config, args.root, args.gcloud)
    try:
